...
import ConstantReorderModal from './ConstantReorderModal';
...
const Dishes: React.FC = () => {
  ...
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc' | 'popularity-desc' | 'popularity-asc' | 'daily-desc' | 'subdaily-desc'>('asc');

  // Reorder Modal State
  const [reorderMode, setReorderMode] = useState<'daily' | 'subDaily' | null>(null);
  const [reorderCategory, setReorderCategory] = useState<string | undefined>(undefined);
  ...
  const sortAndSetDishes = (items: any[], order: string) => {
    ...
    switch (order) {
      ...
      case 'popularity-asc':
        ...
        break;
      case 'daily-desc':
        sorted.sort((a, b) => (b.isDaily ? 1 : 0) - (a.isDaily ? 1 : 0));
        break;
      case 'subdaily-desc':
        sorted.sort((a, b) => (b.isSubDaily ? 1 : 0) - (a.isSubDaily ? 1 : 0));
        break;
    }
    setDishes(sorted);
  };
  ...
  const handleDishSave = (savedItem: MenuItem) => {
    fetchDishes();

    // Check if we need to open reorder modal
    if (savedItem.isDaily) {
      setReorderMode('daily');
      setReorderCategory(savedItem.category);
      setIsReorderModalOpen(true);
    } else if (savedItem.isSubDaily) {
      setReorderMode('subDaily');
      setReorderCategory(savedItem.category);
      setIsReorderModalOpen(true);
    }
  };
  ...
  return (
    <>
              ...
              <div className="relative">
                ...
                >
                  <span className="flex items-center gap-2">
                    ...
                    {sortOrder === 'daily-desc' && <><Zap size={20} className="text-[#C32026]" /> Stałe (Menu)</>}
                    {sortOrder === 'subdaily-desc' && <><Ticket size={20} className="text-[#F28D91]" /> Stałe (Abon)</>}
                  </span>
                </button>
                ...
                {isSortMenuOpen && (
                  <div className="absolute top-full mt-2 left-0 w-64 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="p-2 space-y-1">
                      ...
                      <button onClick={() => { setSortOrder('daily-desc'); setIsSortMenuOpen(false); }} className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-2 font-medium transition-colors ${sortOrder === 'daily-desc' ? 'bg-red-50 text-[#C32026]' : 'hover:bg-gray-50 text-gray-700'}`}>
                        <Zap size={18} className="text-[#C32026]" /> Stałe (Menu) Najpierw
                      </button>
                      <button onClick={() => { setSortOrder('subdaily-desc'); setIsSortMenuOpen(false); }} className={`w-full text-left px-4 py-3 rounded-lg flex items-center gap-2 font-medium transition-colors ${sortOrder === 'subdaily-desc' ? 'bg-red-50 text-[#C32026]' : 'hover:bg-gray-50 text-gray-700'}`}>
                        <Ticket size={18} className="text-[#F28D91]" /> Stałe (Abon) Najpierw
                      </button>
                    </div>
                  </div>
                )}
      ...
      {/* Modal is external to avoid re-renders of the list */}
      <DishFormModal
        isOpen={isAddModalOpen || !!editingDish}
        onClose={() => {
      ...
      {
        isReorderModalOpen && reorderMode && (
          <ConstantReorderModal
            isOpen={isReorderModalOpen}
            onClose={() => setIsReorderModalOpen(false)}
            mode={reorderMode}
            category={reorderCategory}
            items={dishes} // Pass all dishes, modal filters them
            onSave={fetchDishes}
          />
        )
      }
